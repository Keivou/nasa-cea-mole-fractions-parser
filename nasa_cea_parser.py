...
### Relative path to the example .out file which is basically a .txt file
cea_out_file = "your_nasa_cea_output_file.out"
...
def split_to_rows(mole_fraction_list):
    """
    ...
    for item in mole_fraction_list:
...
### Then convert dataframe into a file with extension of the user's selection

# df = pd.DataFrame(aggregated_mole_fractions)
# print(df)

EXPORT_FILENAME = 'cea_mole_fractions_9T9P'
FILE_EXTENSION = 'xlsx' # Options: 'csv', 'xlsx'
...
try:
    ...
    output_path = f'./results/{EXPORT_FILENAME}.{FILE_EXTENSION}'
    ...
    if FILE_EXTENSION == 'csv':
        ...
        print(f"\nError: Unsupported file extension '{FILE_EXTENSION}'. Only 'csv' and 'xlsx' are supported.")
